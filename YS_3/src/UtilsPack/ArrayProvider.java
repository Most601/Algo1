...
public class ArrayProvider {
	...
	public String toString() {
		...
		if (size <= 8000) { // print Only 'small' Arrays
			for (int i=0; i<size; i++) {
				str =  str + "["+ Utils.getDigit(i) + "]->" + this.arr[i] + "\t";
				if ((i+1) % 8 == 0) str = str +"\n"; 
			}
		}
	...
	public void setArraySort() {
		this.backUpArr = new int[size];
		for (int i = 0; i < this.backUpArr.length; i++) {
			this.backUpArr[i] = this.arr[i];
		}
		Arrays.sort(this.arr);
	}
	
	public void removeArraySort() {
		if (this.backUpArr == null) return;
		for (int i = 0; i < backUpArr.length; i++) {
			this.arr[i] = backUpArr[i];
		}
	}
}
